...
public class JdbcGalleryDAO implements GalleryDAO{
	...
	public List selectAll() {
		//내부적으로 jdbc를 사용하지만 다 숨겨놓음 즉, 개발자로 하여금 기존의 jdbc의 상투적 코드를
		//작성할 필요없게 해놓은거 뿐임 기존의 jdbc가 맞음
		List galleryList=jdbcTemplate.query("select * from gallery order by gallery_idx", new RowMapper<Gallery>() {
			...
			public Gallery mapRow(ResultSet rs, int rowNum) throws SQLException {
                ...
                List photoList = jdbcTemplate.query(sql, new Object[] {gallery.getGallery_idx()}, new RowMapper<Photo>() {  //(sql, 바인드변수, 매퍼)
                    public Photo mapRow(ResultSet rs, int rowNum) throws SQLException {
                        Photo photo = new Photo();  //empty
	...
	public void update(Gallery gallery) throws GalleryException {
		String sql="update gallery set title=?, writer=?, content=? where gallery_idx=?";
		int result=jdbcTemplate.update(sql, new Object[] {gallery.getTitle(), gallery.getWriter(), gallery.getContent(), gallery.getGallery_idx()});
	...
	public void delete(int gallery_idx) throws GalleryException{
		...
		int result=jdbcTemplate.update(sql, new Object[] {gallery_idx});
