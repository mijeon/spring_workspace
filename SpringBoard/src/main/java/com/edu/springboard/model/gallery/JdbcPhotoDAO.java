...
public class JdbcPhotoDAO implements PhotoDAO {
	...
	public List selectAll() {
		...
		List<Photo> photoList=jdbcTamplate.query(sql, new RowMapper<Photo>() {
	...
	public Photo select(int photo_idx) {
		...
		Photo photo=jdbcTamplate.queryForObject(sql, new RowMapper<Photo>() {
	...
	public void insert(Photo photo) throws PhotoException {
		...
		int result=jdbcTamplate.update(sb.toString(), new Object[] {photo.getGallery().getGallery_idx(), photo.getFilename()});
		if(result<1) {
	...
	public void deleteByGallery(int gallery_idx) throws PhotoException {
		...
		int result=jdbcTamplate.update(sb.toString(), new Object[] {gallery_idx});
		if(result<1) {
